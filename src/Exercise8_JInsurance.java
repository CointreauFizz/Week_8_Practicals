...
public abstract class Exercise8_JInsurance extends JFrame implements ItemListener {
    ...
    public class JInsurance implements ItemListener {
        ...
        public void itemStateChanged(ItemEvent check) {
            ...
            if (source == hmo) {
                ...
                if (select == ItemEvent.SELECTED) insChosen = "hmo $" + HMOPRICE;
                else if (select == ItemEvent.DESELECTED) insChosen = "";
                output = " " + insChosen;
                insChoice.setText(output);
            }
            if (source == ppo) {
                int select = check.getStateChange();
                if (select == ItemEvent.SELECTED) insChosen = "ppo $" + PPOPRICE;
                else if (select == ItemEvent.DESELECTED) insChosen = "";
                output = " " + insChosen;
                insChoice.setText(output);
            }
            if (source == dental) {
                int select = check.getStateChange();
                if (select == ItemEvent.SELECTED) insChosen = "dental $" + DENTALPRICE + " additional";
                else if (select == ItemEvent.DESELECTED) insChosen = "";
                output = " " + insChosen;
                insChoice.setText(output);
            }
            if (source == vision) {
                int select = check.getStateChange();
                if (select == ItemEvent.SELECTED) insChosen = "vision $" + VISIONPRICE + " additional";
                else if (select == ItemEvent.DESELECTED) insChosen = "";
                output = " " + insChosen;
                insChoice.setText(output);
            }
